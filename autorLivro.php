...
<?php
    ...
    if($id != NULL){        //lógica para alterar valores no BD, se id for diferente de nulo, significa alteração
        $sql="select * from autor_livro where codigo = ". $id;
...
<body>
    ...
    <h1>Cadastro do Autor do Livro</h1>
    <form method="post" action="processar.php" onsubmit="return validarAutorLivro()">
        <table class="tabela3campos">
            <tr>
                ...
                <td>Autor<span class="obrigatorio">*</span><input type="text" placeholder="Nome da Autor" name="autor" value="<?php echo $autor;?>" id="autor" class="campo"></td>
                <td>Livro<span class="obrigatorio">*</span><input type="text" placeholder="Nome do Livro" name="livro" value="<?php echo $livro;?>" id="livro" class="campo"></td>
                <td>              
                    <a href="autorLivro.php"><input type="button"  value="Limpar" id="limpar"></a>
                    <input type="hidden" name="id" value="<?php echo $id?>">
                    <input type="hidden" name="tipo" value="autorLivro"/>
